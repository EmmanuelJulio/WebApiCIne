...
namespace PeliculasAPI.Tests.PruebasUnitarias
{
    ...
    public class CuentasControllerTests : BasePruebas
    {
        ...
        public async Task CrearUsuario()
        {
            ...
            var usuarios=await context2.Users.CountAsync();
            Assert.AreEqual(1, usuarios);
        }
        [TestMethod]
        public async Task UsuarioNoPuedeLoggearse()
        {
            var nombreDB = Guid.NewGuid().ToString();
            await CrearUsuarioHelper(nombreDB);

            var controller = ConstruirCuentasController(nombreDB);
            var userinfo = new CredencialesUsuario() { Email = "emajulio.ej@gmail.com", Password = "MalPaswword" };
            var respuesta = await controller.Loggin(userinfo);
        ...
        [TestMethod]
        public async Task UsuarioSiPuedeLoggearce()
        {
            var nombreDB = Guid.NewGuid().ToString();
            await CrearUsuarioHelper(nombreDB);
            var userInfo = new CredencialesUsuario() { Email = "emajulio.ej@gmail.com", Password = "!A123456a" };
            var controller = ConstruirCuentasController(nombreDB);
            var respuesta = await controller.Loggin(userInfo);
        ...
        private CuentasController ConstruirCuentasController(string nombreBD)
        {
            ...
            {
                {"LlaveJwt", "ASDJKNKJN4KJ5N4KJSNDKAJSNDAKSJDNK4J5N43KJ53N4KSNDJASKLDNAS" }
            };
